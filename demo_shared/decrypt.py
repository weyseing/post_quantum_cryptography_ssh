...
# client public key, ciphertext (nonce + ct)
...
with open(server_priv_pem, "rb") as f:
    server_priv = load_pem_private_key(f.read(), password=None, backend=default_backend())
...
hkdf = HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=b"toy-ssh-session", backend=default_backend())
